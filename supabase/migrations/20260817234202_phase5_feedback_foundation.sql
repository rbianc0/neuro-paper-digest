...
begin
  ...
  insert into public.user_paper_events (
    ...
    v_user_id, p_paper_id, p_digest_id, p_event_type, coalesce(p_metadata, '{}'::jsonb)
  )
...
as $$
  with per_paper as (
    select
      ...
      ((array_agg(e.event_type order by e.created_at desc, e.id::text desc)
        filter (where e.event_type in ('SAVE', 'UNSAVE'))))[1] as save_state,
      ((array_agg(e.event_type order by e.created_at desc, e.id::text desc)
        filter (where e.event_type in ('MORE_LIKE_THIS', 'LESS_LIKE_THIS'))))[1] as explicit_state,
      ((array_agg(e.metadata order by e.created_at desc, e.id::text desc)
        filter (where e.event_type in ('MORE_LIKE_THIS', 'LESS_LIKE_THIS'))))[1] as explicit_metadata,
    ...
    select
      ...
      + (case
           ...
           when pp.explicit_state = 'LESS_LIKE_THIS' then
             case
               when coalesce(pp.explicit_metadata->>'reason', '') = any(coalesce(p_neutral_less_reasons, '{}'::text[])) then 0
               else -p_less_weight
             end
           else 0
  ...
  where p.embedding is not null
    and abs(w.effective_weight) > 1e-9;
$$;
