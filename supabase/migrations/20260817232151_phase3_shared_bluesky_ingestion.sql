...
create index bluesky_post_events_actor_time_idx
  on public.bluesky_post_events (actor_did, signal_timestamp desc);
create index bluesky_post_events_post_uri_idx
  on public.bluesky_post_events (post_uri);
...
create table public.bluesky_scholarly_links (
  ...
  resolution_status text not null default 'PENDING'
    check (resolution_status in ('PENDING', 'RESOLVED', 'UNRESOLVED', 'ERROR')),
...
create index bluesky_scholarly_links_status_idx
  on public.bluesky_scholarly_links (resolution_status, last_attempted_at);
create index bluesky_scholarly_links_paper_idx
  on public.bluesky_scholarly_links (resolved_paper_id)
  where resolved_paper_id is not null;

create trigger bluesky_scholarly_links_set_updated_at
before update on public.bluesky_scholarly_links
...
create function public.replace_user_bluesky_follows(
  p_user_id uuid,
  p_bluesky_did text,
...
create function public.get_stale_bluesky_accounts(
...
set search_path = ''
as $$
  select a.did, a.handle, a.display_name, a.last_feed_fetched_at,
         a.error_count, a.next_fetch_after
  from public.bluesky_accounts a
  where exists (
    select 1
    from public.user_bluesky_follows f
    where f.followed_did = a.did and f.active = true
  )
